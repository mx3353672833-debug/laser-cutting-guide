...
4. 安装 `requirements.txt`，运行 `check_content.py`、`build_site.py` 和 `check_site.py`（位于 `scripts/`）。
5. 提 PR，使用 PR 模板勾选清单。
...
4. Install `requirements.txt`, then run `check_content.py`, `build_site.py` and `check_site.py` under `scripts/`.
